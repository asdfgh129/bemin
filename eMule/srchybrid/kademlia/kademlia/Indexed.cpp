		...
		do{
			...
			while( pos1 != NULL )
			{
				...
				for(POSITION pos2 = pCurrSource->ptrlEntryList.GetHeadPosition(); pos2 != NULL; )
				{
					...
					if ( (bOnlyTrusted ^ (pCurrName->GetTrustValue() < 1.0f)) && (!pSearchTerms || pCurrName->StartSearchTermsMatch(pSearchTerms)) )///snow:¥Ê‘⁄”Îπÿº¸◊÷œ‡πÿÃıƒø
					{
						if( iCount < 0 )   ///snow:’‚∏ˆ «◊ˆ ≤√¥µƒƒÿ£ø
							iCount++;
