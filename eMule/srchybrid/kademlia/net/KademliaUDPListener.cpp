...
void CKademliaUDPListener::Process_KADEMLIA2_SEARCH_RES (const byte *pbyPacketData, uint32 uLenPacket, CKadUDPKey /*senderUDPKey*/, uint32 uIP, uint16 uUDPPort)
{
	...
	theApp.QueueTraceLogLine(TRACE_SEARCH_PROCESS,_T("Function:%hs|Line:%i|uSource:%s|uTarget：%s|uCount:%i"),__FUNCTION__,__LINE__,uSource.ToHexString(),uTarget.ToHexString(),uCount);  ///snow:add by snow
	...
	while( uCount > 0 )
	{
		...
		try
		{
			...
			theApp.QueueTraceLogLine(TRACE_SEARCH_PROCESS,_T("Function:%hs|Line:%i|uAnswer:%s|Tags：%s"),__FUNCTION__,__LINE__,uAnswer.ToHexString(),PrintKagTags(pTags).GetBuffer(0));  ///snow:add by snow
