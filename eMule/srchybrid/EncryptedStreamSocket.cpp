...
void CEncryptedStreamSocket::CryptPrepareSendData(uchar* pBuffer, uint32 nLen){
	...
	{
		///snow:add by snow
		theApp.QueueTraceLogLine(TRACE_STREAM_DATA,_T("Class:CEncryptedStreamSocket|Function:CryptPrepareSendData before Crypt|Socket:%i|IP:%s|Port:%i|Size:%i|Opcode:|Protocol:|Content(Hex):%s|Content:"),m_SocketData.hSocket,GetPeerAddress().GetBuffer(0),GetPeerPort(),nLen,ByteToHexStr(pBuffer,nLen).GetBuffer(0));

		RC4Crypt(pBuffer, pBuffer, nLen, m_pRC4SendKey);

		///snow:add by snow
		theApp.QueueTraceLogLine(TRACE_STREAM_DATA,_T("Class:CEncryptedStreamSocket|Function:CryptPrepareSendData after Crypt|Socket:%i|IP:%s|Port:%i|Size:%i|Opcode:|Protocol:|Content(Hex):%s|Content:"),m_SocketData.hSocket,GetPeerAddress().GetBuffer(0),GetPeerPort(),nLen,ByteToHexStr(pBuffer,nLen).GetBuffer(0));
	}
}
