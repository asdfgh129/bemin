...
int CAsyncSocketEx::Receive(void* lpBuf, int nBufLen, int nFlags /*=0*/)
{
	...
	if (m_pFirstLayer)
		...
		theApp.QueueTraceLogLine(TRACE_STREAM_DATA,_T("Class:CAsyncSocketEx|Function:Receive|Socket:%i|IP:%s|Port:%i|Size:%i|Opcode:|Protocol:|Content(Hex):%s|Content:"),m_SocketData.hSocket,GetPeerAddress().GetBuffer(0),GetPeerPort(),len,ByteToHexStr((uchar*)lpBuf,len).GetBuffer(0));
   return len;
}
...
int CAsyncSocketEx::Send(const void* lpBuf, int nBufLen, int nFlags /*=0*/)
{
	...
	if (m_pFirstLayer)
		...
		///snow:add by snow
		theApp.QueueTraceLogLine(TRACE_STREAM_DATA,_T("Class:CAsyncSocketEx|Function:Send|Socket:%i|IP:%s|Port:%i|Size:%i|Opcode:|Protocol:|Content(Hex):%s|Content:"),m_SocketData.hSocket,GetPeerAddress().GetBuffer(0),GetPeerPort(),nBufLen,ByteToHexStr((uchar*)lpBuf,nBufLen).GetBuffer(0));
