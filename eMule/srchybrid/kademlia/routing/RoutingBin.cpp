...
bool CRoutingBin::AddContact(CContact *pContact)
{
	...
	{
		///snow:add by snow
		theApp.QueueTraceLogLine(TRACE_KAD_BINARY_TREE,_T("Function:%hs|Line:%i|Contact ID:%s|Contact IP:%s|Distance:%s|Contact Num:%i"),__FUNCTION__,__LINE__,pContact->GetClientID().ToHexString(),ipstr(ntohl(pContact->GetIPAddress())),pContact->GetDistance().ToBinaryString(),m_listEntries.size());
