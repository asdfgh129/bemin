...
///snow:CUInt128µÄÎ»ÊýÊÇ´Ó×óÍùÓÒÊýµÄ£¬²»ÊÇ´ÓÓÒÍù×óÊýµÄ£¡£¡£¡
...
CRoutingZone::CRoutingZone(LPCSTR szFilename)
{
	...
	///snow:¸¸½áµãÎªNULL£¬±íÊ¾Îª¸ù½áµã£¬zoneindexÎª0
	// Init our root node.
	Init(NULL, 0, CUInt128((ULONG)0));
}
...
{
	// Create a new leaf.  ///snow:½¨Á¢Ò»¸öÃ»ÓÐ×ÓÊ÷µÄÒ¶×Ó½áµã
...
void CRoutingZone::Init(CRoutingZone *pSuper_zone, int iLevel, const CUInt128 &uZone_index)
{
	...
	///snow:³õÊ¼»¯Ê±×óÓÒ×ÓÊ÷¾ùÎª¿Õ£¬ÎªÒ¶×Ó½áµã
...
CRoutingZone::~CRoutingZone()
{
	...
	if (IsLeaf())   ///snow:Ò¶×Ó½áµã£¬É¾³ým_pBin£¬·ÇÒ¶½áµã£¬É¾³ý×óÓÒ×ÓÊ÷
		delete m_pBin;
	else
	{
...
*****************************************************snow:end******************************************************/
		...
		if (file.Open(strSpecialNodesdate.IsEmpty() ? m_sFilename : strSpecialNodesdate, CFile::modeRead | CFile::osSequentialScan|CFile::typeBinary|CFile::shareDenyWrite, &fexp))
		{
				...
				if (file.GetLength() >= 8){
					...
					if (uVersion == 3){
						...
						if (nBoostrapEdition == 1){
							...
							///snow:bootstrapNodeDatµÄÇ°12¸ö×Ö½ÚÊÇ 00 00 00 00 03 00 00 00 01 00 00 00
			...
			///snow:Õâ±ßÓÐÁ½ÖÖÇé¿ö£º1ÊÇÇ°ËÄ¸ö×Ö½Ú²»ÊÇ00 00 00 00£¬ÊÇ¾É°æ±¾µÄNodes.dat,uNumContacts!=0£¬ºó±ßÊÇÃ¿¸öÁªÏµÈË25×Ö½Ú
			///snow:                2ÊÇÇ°ËÄ¸ö×Ö½ÚÊÇ00 00 00 00£¬¾­¹ýÉÏÃæ´úÂëµÄ´¦Àí£¬uNumContactsÒ²±»¸³ÖµÁË£¬ºó±ßÊÇÃ¿¸öÁªÏµÈË34×Ö½Ú
...
///snow:Òýµ¼³ÌÐò£¬¶ÁÈ¡µÄ½ÚµãÐÅÏ¢´æÈës_liBootstapList£¬ÔÚCKademlia::Process()ÖÐ´¦Àí
...
///snow:ÔÚÎö¹¹µÄÊ±ºò½«KADIDÐ´ÈëÎÄ¼þ
...
{
	// Max levels allowed.
if (m_uLevel >= 127)   ///snow:×î´ó²ãÊý127£¬²»ÄÜÔÙ·Ö¸îÁË
		return false;

	// Check if this zone is allowed to split.
if ( (m_uZoneIndex < KK || m_uLevel < KBASE) && m_pBin->GetSize() == K)///snow:ºóÃæµÄÌõ¼þKÍ°ÂúÁËºÃÀí½â£¬Ç°ÃæµÄÊÇÌõ¼þÊÇÊ²Ã´ÒâË¼£¿±íÊ¾IndexÎª000£¬001£¬010£¬011£¬110µÄKÍ°µÄ·ÖÁÑ²»ÊÜÏÞÖÆ£¬¿ÉÒÔÒ»Ö±·ÖÁÑµ½Level=127£¬¶øindex>5µÄKÍ°Ö»ÄÜ·ÖÁÑµ½µÚ4²ã
		return true;
	return false;
}
...
{
	if (uID != uMe && uVersion > 1)   ///snow:uMeÔÚ¹¹Ôìº¯ÊýÖÐ¸³Öµ£ºGetPrefs()->GetKadID(&uMe)£¬²»ÊÇ×ÔÉí£¬ÇÒ°æ±¾Îª2ÒÔÉÏ°æ±¾
	{
		CContact* pContact = new CContact(uID, uIP, uUDPPort, uTCPPort, uVersion, cUDPKey, bIPVerified);
		if (bFromHello)
			pContact->SetReceivedHelloPacket();  ///snow:ÕâÀïÉèÖÃµÄÖµÔÚAdd()ÖÐÈ¡ÖµÅÐ¶Ï
	...
	else  ///snow:µ±Ç°ÊÇÒ¶×Ó½áµã
	{
		...
		if (pContactUpdate)   ///snow:ÒÑ¾­´æÔÚÏàÍ¬KadIDµÄÁªÏµÈË
		{
			if(bUpdate)
			{
				...
				{
					///snow:UDPKey!=00(version 4µÄUDPKeyÊÇ00£©ÇÒÐÂÒªÇóÔö¼ÓµÄÁªÏµÈËµÄUDPKey!=µ±Ç°ÒÑ´æÔÚµÄÁªÏµÈËµÄUDPKey
				...
				///snow:°æ±¾ºÅÔÚ0.46c--0.49abetaÖ®¼ä£¬ÇÒÊÕµ½ÁËHelloPacket
					...
					&& pContactUpdate->GetReceivedHelloPacket())
				{
					...
					if (m_pBin->ChangeContactIPAddress(pContactUpdate, pContact->GetIPAddress())   ///snow:º¯ÊýÃûÆðµÄÓÐµãÎóµ¼£¬²»ÊÇChange£¬¶øÊÇCanChange
						&& pContact->GetVersion() >= pContactUpdate->GetVersion()) // do not let Kad1 responses overwrite Kad2 ones///snow:ÐÂ°æ±¾µÄ¿ÉÒÔÌæ»»¾É°æ±¾µÄ,KAD2¿ÉÒÔÌæ»»KAD1µÄ£¬·´¹ýÀ´²»ÐÐ
					{
		...
		else if (m_pBin->GetRemaining())  ///snow:½áµãÖÐ»¹ÓÐÊ£Óà¿Õ¼ä£¬ÁªÏµÈË¸öÊýÎ´¹ýµ½KÖµ
		{
			bUpdate = false;
		...
		else if (CanSplit())///snow:½áµãÖÐÃ»ÓÐ¿Õ¼ä´æ·ÅÐÂµÄÁªÏµÈËÁË£¬ÅÐ¶ÏÒ»ÏÂÊÇ·ñ¿ÉÒÔ²ð·Ö½áµã
		{
...
{
	if (IsLeaf())   ///snow:Èç¹ûÊÇÒ¶×Ó½áµã£¬Ôòµ÷ÓÃGetContact()º¯Êý
		return m_pBin->GetContact(uID);
	else{     ///snow:Èç¹ûÊÇ·ÖÖ§½áµã£¬Ôò¸ù¾ÝuDistanceÖÐÓë±¾zoneµÄm_uLevel¶ÔÓ¦µÄÎ»µÄÖµ£¬·Ö±ðµ÷ÓÃ×ó×ÓÊ÷»òÓÒ×ÓÊ÷£¬Ö±µ½Ò¶×Ó½áµã£¬×îºó»¹ÊÇµ÷ÓÃÒ¶×Ó½áµãµÄGetContact()
		CUInt128 uDistance;
		CKademlia::GetPrefs()->GetKadID(&uDistance); ///snow:½«±¾»úKadID¸³Öµ¸øuDistance
		uDistance.Xor(uID);   ///snow:½«uIDÓë±¾»úKADID½øÐÐÒì»ò£¬µÃµ½¾àÀë
...
///snow:×î½üµÄÁªÏµÈË×ÜÊÇÓëÄ¿±ê½áµãµÄÎ»×î´ó³Ì¶ÈµÄÒ»ÖÂ(´Ó¸ßÎ»ÍùµÍÎ»Æ¥Åä(ÒòÎªCUInt128¶¨ÒåÁË×î¸ßÎ»ÊÇ0Î»))£¬Èç¹û±¾½áµãÊÇÒ¶×Ó½áµã£¬Ôò¾ÍÊÇ½áµãÖÐ´æ´¢µÄÁªÏµÈË£¬Èç¹û²»ÊÇ£¬Ôò°´Î»Ë÷Òý·ÖÖ§½áµã£¬Ö±µ½Ò¶×Ó½áµã
///snow:GetClosestTo()ÖÐ²ÎÊýuDistanceÔò´ú±íµÄÊÇ´ÓRoutingZone¶¥µã¿ªÊ¼£¬¸ù¾ÝuDistanceµÄÖµ°´Î»Ë÷Òý²éÕÒÒ¶×Ó½áµã
///snow:m_uTarget²ÎÊýÔòÊÇÎªÁË¼ÆËã²éÕÒ³öµÄ´ý¶¨½ÚµãÓëÄ¿±ê½ÚµãµÄ¾àÀë£¬È»ºó½«¾àÀëÖµ×÷ÎªË÷ÒýÐ´ÈëContactMap
void CRoutingZone::GetClosestTo(uint32 uMaxType, const CUInt128 &uTarget, const CUInt128 &uDistance, uint32 uMaxRequired, ContactMap *pmapResult, bool bEmptyFirst, bool bInUse) const
{
	...
	///snow:uDistancceµÄ×÷ÓÃÊÇ°´Î»Æ¥ÅäÑ°ÕÒ·ÖÖ§½Úµã
	///snow:°´Î»Æ¥Åä£¬Ñ°ÕÒÏàÓ¦µÄ·ÖÖ§½áµã£¬Èç¹ûµ±Ç°·ÖÖ§°Ñ·µ»ØµÄ½á¹ûÊý½ÏÉÙ£¬ÔÙ´ÓÁíÒ»¸ö×ÓÊ÷ËÑË÷
...
void CRoutingZone::GetAllEntries(ContactList *pmapResult, bool bEmptyFirst)
{
	...
	else
	{
		...
		m_pSubZones[1]->GetAllEntries(pmapResult, false);///snow:²»Çå¿ÕÁÐ±í£¬ÒòÎª´æ´¢ÁË×ó×ÓÊ÷µÄ½áµã
	}
}
...
void CRoutingZone::Split()
{
	...
	m_pBin->GetEntries(&listEntries); ///snow:½«Ô­ÏÈ´æ´¢ÔÚm_pBin->m_listEntriesÖÐµÄ¶ÔÏóµÄÖ¸Õë¸´ÖÆµ½listEntriesÖÐ
	m_pBin->m_bDontDeleteContacts = true;  ///snow:Õâ¸ö²ÎÊýÓ¦¸ÃÊÇ¸øCRouteingBinµÄÎö¹¹º¯ÊýÓÃµÄ
	delete m_pBin;
	m_pBin = NULL;	

	///snow:Ô­ÏÈ´æ´¢ÔÚm_listEntriesÖÐµÄÃ¿¸öContactList¶ÔÏó¶¼·ÖÅäÓÐ×Ô¼ºµÄÒ»¶ÎÄÚ´æ£¬ÓÐÒ»¸öÖ¸ÕëÖ¸ÏòÕâ¸öÄÚ´æ£¬¶øÕâ¸öÖ¸Õë¾Í´æ´¢ÔÚm_listEntriesÖÐ£¬
	///snow:Êµ¼ÊµÄContactList¶ÔÏóÊÇ²»´æ´¢ÔÚm_listEntriesÖÐµÄ£¬Ò²¾ÍÊÇËµm_listEntriesÊµ¼ÊÊÇ¸öÖ¸Õë¼¯ºÏ£¬ËùÒÔ¿ÉÒÔ°ÑÕâÐ©Ö¸Õë¿½³öÀ´£¬ÔÙÉ¾³ýµôm_pBin¶ÔÏó
	///snow:¶ø²»Ó°ÏìContactList¶ÔÏó£¬µ±È»£¬Ò²¿ÉÒÔÔÚÎö¹¹º¯ÊýÖÐÉ¾³ýµôÊµ¼ÊµÄContactList¶ÔÏó
	
	///snow:½«Ô­ÏÈ½áµãÖÐµÄÁªÏµÈË·ÖÅäµ½×óÓÒ×ÓÊ÷ÖÐ£¬¸ù¾ÝµÄÊÇm_uDistanceÖÐm_uLevel¶ÔÓ¦µÄÎ»µÄÖµ£¬0Ôò×ó×ÓÊ÷£¬1ÔòÓÒ×ÓÊ÷
	for (ContactList::const_iterator itContactList = listEntries.begin(); itContactList != listEntries.end(); ++itContactList)
	{
		...
		if (!m_pSubZones[iSuperZone]->m_pBin->AddContact(*itContactList))  ///snow:AddContact()ÖÐÌí¼ÓµÄÒ²ÊÇÖ¸Õë
...
uint32 CRoutingZone::Consolidate()
{
	...
	if( m_pSubZones[0]->IsLeaf() && m_pSubZones[1]->IsLeaf() && GetNumContacts() < K/2 )
	{
		...
		///snow:½«Ô­×óÓÒ×ÓÊ÷µÄÁªÏµÈËÖ¸ÕëÌí¼Óµ½ÐÂÒ¶×Ó½áµãµÄÁªÏµÈËÁÐ±íÖÐ
...
{
	return (m_pBin != NULL);   ///snowL:ÔÚsplit()ÖÐ£¬delete m_pBin;m_pBin = NULL;
}

CRoutingZone *CRoutingZone::GenSubZone(int iSide)
{
	CUInt128 uNewIndex(m_uZoneIndex);   ///snow:¼ÙÉè¸¸½áµãµÄm_uZoneIndexÎª0£¬ÔòÐÂ½¨µÄÒ¶×Ó½áµã£¬×óÒ¶×Ó½áµãm_uZoneIndex=00£¬ÓÒÒ¶×Ó½áµãm_uZoneIndexÎª01£¬ÒÀ´ËÀàÍÆ£¬Îª010£¬011£¬100£¬101£¬1000£¬1001£¬ÓÐµãÎÊÌâÑ½£¿²»ÊÇºÜ¶Ô£¬ÄÇ0µÄÄÇÖ§£¬ÔÙÔõÃ´×óÒÆ¶¼ÊÇ0
	///snow:Êµ¼Ê0µÄÒ¶×Ó½áµãÖ»ÓÐÒ»¸ö£¬PPTÀïÓÐÏêÏ¸µÄ·ÖÁÑ¹ý³Ì
	uNewIndex.ShiftLeft(1);
	if (iSide != 0)
		uNewIndex.Add(1);
	return new CRoutingZone(this, m_uLevel+1, uNewIndex); ///snow:this±íÊ¾ÐÂµÄzoneÎª±¾zoneµÄ×Ózone
}

///snow:½«µ±Ç°ZOne¼ÓÈëm_mapEvent£¬ÔÚCKademlia::Process()ÖÐ½øÐÐ¸üÐÂ´¦Àí¡£ÔÚInit()ºÍConsolidate()ÖÐµ÷ÓÃ
...
///snow:ÔÚSplit()ºÍConsolidate()ÖÐµ÷ÓÃ
...
bool CRoutingZone::OnBigTimer()
{
	...
	{
		RandomLookup();   ///snow:Ëæ»ú²éÕÒ½Úµã
		return true;
	}
