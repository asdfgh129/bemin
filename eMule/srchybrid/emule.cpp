...
/*********************************************** by snow start
这段代码在“内存泄漏分析及常用内存泄漏检测工具分享 ”的第四节有详细介绍
...
CLogFile theDebugLog;   ///snow:add by snow
...
////by snow start
//  CMemoryState定义在afx.h中，是MFC的一个内存调试类
...
void InitDEP()
{
	...
	////  by snow start
	//  GetProcessDEPPolicy和SetProcessDEPPolicy函数为kernel32.dll中的函数，通过WinApi函数GetProcAddress和GetModuleHandle获取
    ////  by snow end
