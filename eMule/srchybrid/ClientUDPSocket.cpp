...
int CClientUDPSocket::SendTo(char* lpBuf,int nBufLen,uint32 dwIP, uint16 nPort){
	...
	///snow:add by snow
	theApp.QueueTraceLogLine(TRACE_STREAM_DATA,_T("Function:%s|Line:%i|Socket:|IP:%s|Port:%i|Size:%i|Opcode:|Protocol:|Content(Hex):%s|Content:%s"),__FUNCTION__,__LINE__,ipstr(ntohl(dwIP)),nPort,nBufLen,ByteToHexStr((uchar*)lpBuf,nBufLen).GetBuffer(0),TrimZero((uchar*)lpBuf,nBufLen).GetBuffer(0));
