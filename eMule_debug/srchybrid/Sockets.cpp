...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		theApp.QueueDebugLogLine(false,GetResString(IDS_CONNECTEDTOREQ), sender->cur_server->GetListName(), sender->cur_server->GetAddress(), sender->IsServerCryptEnabledConnection() ? sender->cur_server->GetObfuscationPortTCP() : sender->cur_server->GetPort());
		...
		if (thePrefs.GetDebugServerTCPLevel() > 0)
		{
			...
			theApp.QueueDebugLogLine(false,_T(">>> Sending OP__LoginRequest\n"));
		}
	...
	else if (sender->GetConnectionState() == CS_CONNECTED)
	{
		...
		if (thePrefs.GetAddServersFromServer())
		{
			...
			if (thePrefs.GetDebugServerTCPLevel() > 0)
				theApp.QueueDebugLogLine(false,_T(">>> Sending OP__GetServerList\n"));
			theStats.AddUpDataOverheadServer(packet->size);
